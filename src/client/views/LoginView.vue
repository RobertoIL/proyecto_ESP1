<template>
    <main class="bg-login-background bg-cover">
        <div class="flex justify-center p-8 bg-gray-800 bg-opacity-90 h-screen">
            <div class="bg-slate-200 bg-opacity-50 rounded-xl p-8 w-[450px] h-fit">
                <p class="text-3xl text-center font-bold pb-8 font-font_tittle">Iniciar Sesión</p>
                <form class="" @submit.prevent="handleLogin">
                    ...
                    <div class="flex flex-col space-y-4">
                        <RouterLink class="hover:text-blue-800 underline" to="/register">No tengo cuenta</RouterLink>
...
const handleLogin = async () => {
  ...
  try {
    ...
    if (error.response && error.response.data) {
      ...
      console.log('Datos de error recibidos:', error.response.data);
    }
  }
};
</script>
