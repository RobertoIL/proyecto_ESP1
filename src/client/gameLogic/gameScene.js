export default class GameScene extends Phaser.Scene {
  ...
  preload() {
    this.load.spritesheet("player1_walk", this.rutaP1, {
      frameWidth: this.walk1.size,
      frameHeight: this.walk1.size,
    });
    this.load.spritesheet(
      "player1_attack",
      this.rutaP1.replace("Walk", "Attack"),
      { frameWidth: this.attack1.size, frameHeight: this.attack1.size }
    );

    this.load.spritesheet("player2_walk", this.rutaP2, {
      frameWidth: this.walk2.size,
      frameHeight: this.walk2.size,
    });
    this.load.spritesheet(
      "player2_attack",
      this.rutaP2.replace("Walk", "Attack"),
      { frameWidth: this.attack2.size, frameHeight: this.attack2.size }
    );
  ...
  create() {
    ...
    this.hitbox = [
      new Hitboxes(this.player1Class),
      new Hitboxes(this.player2Class),
    ];

    this.player1Sprite.setSize(
      this.hitbox[0].hitbox.vertical_walk.width,
      this.hitbox[0].hitbox.vertical_walk.height
    );
    this.player1Sprite.setOffset(
      this.hitbox[0].hitbox.vertical_walk.offsetX,
      this.hitbox[0].hitbox.vertical_walk.offsetY
    );

    this.player2Sprite.setSize(
      this.hitbox[1].hitbox.vertical_walk.width,
      this.hitbox[1].hitbox.vertical_walk.height
    );
    this.player2Sprite.setOffset(
      this.hitbox[1].hitbox.vertical_walk.offsetX,
      this.hitbox[1].hitbox.vertical_walk.offsetY
    );
    ...
    this.player1Sprite.on("animationupdate", (animation, frame) => {
      if (
        animation.key.includes("right") &&
        animation.key.includes("attack") &&
        frame.index >= this.attack1.total - 2 &&
        !(this.player1Class == "Archer") &&
        !(this.player1Class == "Mage")
      ) {
      ...
      if (
        animation.key.includes("attack") &&
        frame.index == this.attack1.total - 4 &&
        this.player1Class == "Archer"
      ) {
        this.dispararFlecha(
          this.player1Sprite,
          animation.key.replace("attack_", "").replace("_1", ""),
          this.player2Sprite
        );
      }
      if (
        animation.key.includes("attack") &&
        frame.index == this.attack1.total &&
        this.player1Class == "Mage"
      ) {
        this.lanzarHechizo(
          this.player1Sprite,
    ...
    this.player2Sprite.on("animationupdate", (animation, frame) => {
      if (
        animation.key.includes("right") &&
        animation.key.includes("attack") &&
        frame.index >= this.attack2.total - 2
      ) {
        if (this.player2Class == "Lancer") {
          this.updateAttackHitbox(
            1,
            this.player2Sprite.x,
            this.player2Sprite.y,
            60,
            15,
            42,
            12.5
      ...
      if (
        animation.key.includes("attack") &&
        frame.index == this.attack2.total - 4 &&
        this.player2Class == "Archer"
      ) {
        this.dispararFlecha(
          this.player2Sprite,
          animation.key.replace("attack_", "").replace("_2", ""),
          this.player1Sprite
        );
      }
      if (
        animation.key.includes("attack") &&
        frame.index == this.attack2.total &&
        this.player2Class == "Mage"
      ) {
        this.lanzarHechizo(
          this.player2Sprite,
          animation.key.replace("attack_", "").replace("_2", ""),
  ...
  init(data) {
    ...
    this.rutaP1 = "public/" + player1 + "/Walk.png";
    this.rutaP2 = "public/" + player2 + "/Walk.png";
  ...
  updateAttackAnimation(player, indexDirections, indexPlayer) {
    ...
    if (nombreAnim.includes("left") || nombreAnim.includes("right")) {
      player.setSize(
        this.hitbox[indexDirections].hitbox.horizontal_attack.width,
        this.hitbox[indexDirections].hitbox.horizontal_attack.height
      );
      player.setOffset(
        this.hitbox[indexDirections].hitbox.horizontal_attack.offsetX,
        this.hitbox[indexDirections].hitbox.horizontal_attack.offsetY
      );
    } else {
      player.setSize(
        this.hitbox[indexDirections].hitbox.vertical_attack.width,
        this.hitbox[indexDirections].hitbox.vertical_attack.height
      );
      player.setOffset(
        this.hitbox[indexDirections].hitbox.vertical_attack.offsetX,
        this.hitbox[indexDirections].hitbox.vertical_attack.offsetY
      );
    }
    if (
      (this.player1Class == "Saber" && indexPlayer == 1) ||
      (this.player2Class == "Saber" &&
        indexPlayer == 2 &&
        !this.isAttacking[indexDirections])
    ) {
    ...
    player.once(
      ...
      (anim) => {
        if (anim.key === nombreAnim) {
          ...
          if (nombreAnim.includes("left") || nombreAnim.includes("right")) {
            player.setSize(
              this.hitbox[indexDirections].hitbox.horizontal_walk.width,
              this.hitbox[indexDirections].hitbox.horizontal_walk.height
            );
            player.setOffset(
              this.hitbox[indexDirections].hitbox.horizontal_walk.offsetX,
              this.hitbox[indexDirections].hitbox.horizontal_walk.offsetY
            );
          } else {
            player.setSize(
              this.hitbox[indexDirections].hitbox.vertical_walk.width,
              this.hitbox[indexDirections].hitbox.vertical_walk.height
            );
            player.setOffset(
              this.hitbox[indexDirections].hitbox.vertical_walk.offsetX,
              this.hitbox[indexDirections].hitbox.vertical_walk.offsetY
            );
          }
          if (
            (this.player1Class == "Saber" && indexPlayer == 1) ||
            (this.player2Class == "Saber" && indexPlayer == 2)
          ) {
            player.setPosition(player.x, player.y - 39);
          }
  ...
  drawHealthBar(
    ...
    if (x >= this.cameras.main.width / 2) {
      ...
      graphics.fillRect(
        x + (width + x - (width * healthPercentage + x)),
        y,
